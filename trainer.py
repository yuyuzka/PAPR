...
def train(model, max_len, train_data, train_sampler, train_bsz, train_num_neg, num_epoch, quadkey_processor, loc2quadkey,
    ...
    writer=SummaryWriter("loss\\PAPR")
    for epoch_idx in range(num_epoch):
        ...
        model.train()
        for batch_idx, (src_locs_, src_quadkeys_, src_times_, w_mat_, c_mat_, trg_locs_, trg_quadkeys_, data_size) in batch_iterator:
            src_loc = src_locs_.to(device)
            src_quadkey = src_quadkeys_.to(device)
            src_time = src_times_.to(device)
            w_mat = w_mat_.to(device)
            c_mat = c_mat_.to(device)
            ...
            #                trg_loc, trg_quadkey, key_pad_mask, mem_mask, data_size)
            prob, order = src_out.topk(k=1, dim=-1, largest=True)
        ...
        f.close()
    model.save(model_path+str(num_epoch)+".pth")    
    # torch.save(model.state_dict(), model_path+str(num_epoch)+".pth")
